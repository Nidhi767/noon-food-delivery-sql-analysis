...
--find top 3 outletts by cuisine type without using limit an dtop function
with cte as (
select cuisine, restaurant_id, count(*) as number_of_orders
from orders
group by cuisine, restaurant_id)
select * from(
select *,
row_number() over(partition by cuisine order by number_of_orders) as rn
from cte) as a
where rn <= 3;
...
--3. count of all users who acquired in jan 2025 and only placed one order in jan and did not place anny other order in jan
...
--4.list all the customers with no order in the last 7 days but were acquired one month ago with 
...
JOIN orders o
    ON co.customer_code = o.customer_code
   AND co.first_order_date = o.placed_at
WHERE co.latest_order_date < CURRENT_DATE - INTERVAL '7 days'
  AND co.first_order_date <= CURRENT_DATE - INTERVAL '1 month'
ORDER BY co.customer_code;


--5. grwoth team is planning to create a trigger that will target customers after 
--their every third order with a personalized communication and they have asked you
...
--6. list the customers who has placed only one order and all their orders on a promo only
...
--7. what perecent of orders were organically acquired in jan 2025
